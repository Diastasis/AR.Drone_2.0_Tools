...
#        xp1 = np.linspace(dists1.min(),dists1.max(),100) #ORIGINAL
        xp1 = np.linspace(dists1.min(),15,100)
...
#        plt.plot(np.linspace(1,45,num=121),_dbm2,color='pink')
...
#        xp2 = np.linspace(dists2.min(),dists2.max(),100) # ORIGINAL
        xp2 = np.linspace(dists2.min(),15,100)
...
#        ax.set_yscale('log') 
    ...
    def FSPL_dB(obj):
        ...
        freq = 2412 #(Mhz) Anafi = channel 1
        const = -27.55
        dist = obj.distances[0].reshape(1,-1)[0]
        fspl = 20*np.log10(dist)+20*np.log10(freq)+const
        fspl = np.short(fspl)
        plt.scatter(dist,fspl)
        plt.show()
        
        
    def FSPL(obj):
        '''Calculate the Free Space Path Loss (in dB)'''
        f = 2412 #(Mhz) Anafi = channel 1
        pi = 3.14
        dist = obj.distances[0].reshape(1,-1)[0]
        lamda = 0.1242548 # m
        c = 299792458
#        fspl = ((4*pi*dist*f)/c)*2
        fspl = ((4*pi*dist)/lamda)**2
#        fspl = np.short(fspl)
        plt.scatter(dist,fspl)
        plt.show()
...
#    overDistanceLogPlot(ardrone,bitrate_mbs)
#    overRSSPlot(anafi,minTime_ms)
#    overBitratePlot(ardrone,RSS_dbm)
#    dbm(anafi)
#    FSPL(anafi)
