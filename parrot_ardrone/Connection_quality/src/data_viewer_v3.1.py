...
anPkgTx             = (anafiLayers[0, 0], anafiLayers[0, 1], anafiLayers[0, 0]) # OK
anPkgRx             = (anafiLayers[1, 0], anafiLayers[1, 1], anafiLayers[1, 0]) # OK
anPkgLoss_num       = (anafiLayers[2, 0], anafiLayers[2, 1], anafiLayers[2, 0]) # OK
anPkgLoss_perc      = (anafiLayers[3, 0], anafiLayers[3, 1], anafiLayers[3, 0]) # OK
anTimeMin           = (anafiLayers[4, 0], anafiLayers[4, 1], anafiLayers[4, 0]) # OK
anTimeAvg           = (anafiLayers[5, 0], anafiLayers[5, 1], anafiLayers[5, 0]) # OK
anTimeMax           = (anafiLayers[6, 0], anafiLayers[6, 1], anafiLayers[6, 0]) # OK
anTimeDiv           = (anafiLayers[7, 0], anafiLayers[7, 1], anafiLayers[7, 0]) # OK
anPkgDouble_num     = (anafiLayers[8, 0], anafiLayers[8, 1], anafiLayers[8, 0]) # OK
anPkgDouble_perc    = (anafiLayers[9, 0], anafiLayers[9, 1], anafiLayers[9, 0]) # OK
anWifiReadings      = (anafiLayers[10,0], anafiLayers[10,1], anafiLayers[10,0]) # OK
anWifiQuality_num   = (anafiLayers[11,0], anafiLayers[11,1], anafiLayers[11,0]) # OK
anWifiQuality_perc  = (anafiLayers[12,0], anafiLayers[12,1], anafiLayers[12,0]) # OK
anWifiSig_dbm       = (anafiLayers[13,0], anafiLayers[13,1], anafiLayers[13,0]) # OK
anWifiSig_mw        = (anafiLayers[14,0], anafiLayers[14,1], anafiLayers[14,0]) # OK
anWifiNoise_dbm     = (anafiLayers[15,0], anafiLayers[15,1], anafiLayers[15,0]) # OK
anWifiNoise_mw      = (anafiLayers[16,0], anafiLayers[16,1], anafiLayers[16,0]) # OK
anWifiBitrate       = (anafiLayers[17,0], anafiLayers[17,1], anafiLayers[17,0]) # OK
...
titles = ('Packets Transmitted (#)', 'Packets Received (#)', 'Packet Loss (#)','Packet Loss (%)',
            ...
            'Noise Level (dBm)', 'Noise Level (mW)','Bitrate (Mb/s)')
...
#print("len of difDataList: ", difDataList.shape)
#print("difDataList: ",difDataList)
...
for ii in range(9):
    ...
    if ii+1 < 4:
        ax.set_title("AR.Drone 2.0 (Layer {})\n{}".format(ii+1, titles[17]))
        imgplot = ax.imshow(arDataList[12][ii], cmap="inferno",interpolation='gaussian') # interpolation='bicubic', origin='lower', interpolation='gaussian'
#        imgplot.set_clim(60, 100)
        ...
        for i in range(11):
            for j in range(11):
                ...
                ax.text(j, i, arDataList[12][ii][i][j], ha="center", va="center", color="w")
#                ax.text(j, i, anafiLayers[13,2,i, j], ha="center", va="center", color="w")
    elif ii < 6:
        ax.set_title("Anafi (Layer {})\n{}".format(ii-2, titles[17]))
        imgplot = ax.imshow(anDataList[12][ii-4], cmap="inferno",interpolation='gaussian') # interpolation='bicubic', origin='lower', interpolation='gaussian'
#        imgplot.set_clim(60, 100)
        fig.colorbar(imgplot, ax=ax,orientation='horizontal',shrink=0.76,pad=0.08)
        for i in range(11):
            for j in range(11):
                ax.scatter(i,j, alpha=0.1, s=800, color="w") # cmap='jet'
                ax.text(j, i, anDataList[12][ii-4][i][j], ha="center", va="center", color="w")
#                ax.text(j, i, anafiLayers[13,2,i, j], ha="center", va="center", color="w")
    else:
        ax.set_title("Difference (Layer {})\n{}".format(ii-5, titles[17]))
        imgplot = ax.imshow(difDataList[12][ii-6], cmap="inferno",interpolation='gaussian') # interpolation='bicubic', origin='lower', interpolation='gaussian'
#        imgplot.set_clim(60, 100)
        fig.colorbar(imgplot, ax=ax,orientation='horizontal',shrink=0.76,pad=0.08)
        for i in range(11):
            for j in range(11):
                ax.scatter(i,j, alpha=0.1, s=800, color="w") # cmap='jet'
                ax.text(j, i, difDataList[12][ii-6][i][j], ha="center", va="center", color="w")
#                ax.text(j, i, anafiLayers[13,2,i, j], ha="center", va="center", color="w")
fig.savefig("Bitrate.png", dpi=75)  # results in 160x120 px image
plt.show() 
#
#index = 3            
