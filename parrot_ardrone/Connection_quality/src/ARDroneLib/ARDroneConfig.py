...
def activate_navdata(activate=True):
    "Prepare the drone so it can send navdata back to us"
    if activate:    
...
def nervosity_level(percentage=20):
    ...
    euler = int(0.52*percentage)/100.0 # 2 digits after comma
...
def set_ultrasound(freq=0):
    ...
    if freq:    
        return [("pic:ultrasound_freq","7")]
    else:       
        return [("pic:ultrasound_freq","8")]
    
def record_video(activate = True):
    "Start/Stop the recording of a video on the USB key"
    if activate:    
        return[("video:video_on_usb","true"),("video:video_codec","130")]
    else:           
...
# LED animation
def leds(animation=1,freq=2.0,duration=2):
    return[("leds:leds_anim",str(animation)+","+str(float2dec(freq)))+","+str(duration)]
...
SUPPORTED_CONFIG = {
    ...
    "record_video":record_video, "leds":leds
    }
