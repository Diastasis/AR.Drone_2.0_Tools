...
# Function Definition
# =============================================
def pc2drone_pinger(target_ip="192.168.1.1", count=5):
...
#    File paths
    ...
    json_file_path = "/home/ros/parrot2_ws/src/parrot_ardrone/Connection_quality/src/JSON/ConQlt_{}_(0.5m).json".format(time_stamp)
...
#    drone_output_list = []
    ...
    for h in range(h_points):
        npyPath = commonPath + 'NPY/PC2DR_{}_(0.5m)_({}m).npy'.format(time_stamp,(h*h_step)+h_step) # when the height changes creat a new npy file (change path)
...
#                    drone_result = {'z':round((h*h_step)+h_step,2),'x':round((w*w_step)+w_step,2),'y':round((l*l_step)+l_step,2)} # Add coordinate points
    ...
    #                PC results
                    pc_result.update(pc2drone_pinger("192.168.42.1",25))                                  # "127.0.0.1",'192.168.42.98'
...
#                    output_dict['drone2pc'] = drone_output_list
                    ...
                    if savecsv_mode:
                        csvPath_pc = commonPath + "csv/PC2DR_{}_(0.5m).csv".format(time_stamp)
#                        csvPath_drone = commonPath + "csv/DR2PC_{}.csv".format(time_stamp)
                        csvPath_networks = commonPath + "csv/NET_{}(0.5m).csv".format(time_stamp)
