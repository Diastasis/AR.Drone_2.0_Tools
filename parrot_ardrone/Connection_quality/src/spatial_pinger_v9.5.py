...
# Flag variables
#================================
dbg_mode = True     # Enable debugging
...
def drone2pc_pinger(telnet_host="192.168.1.1", target_ip="192.168.1.3", _count = 5):
    ...
    try:
        tn = telnetlib.Telnet(telnet_host,23,10)
    except OSError:
    ...
    OUTPUT = tn.expect([ping_pattern], 10)                              # Time out in 60 Secs
...
#                    point_wifi_list.append(item)
#                    pc_wifi_result.update(item)
                    wifi_output_list.append(item)
                
#                pc_wifi_result['wifi'] = iwlist.parse(content)
...
#                wifi_output_list.append(pc_wifi_result)
        ...
        #  ==========[ Export NPY file  ]==========
        if savenpy_mode:
            npyPath = commonPath + 'NPY/Ping_{}_({}m).npy'.format(time_stamp,(h*h_step)+h_step)
            np.save(npyPath, point_data)
            print('\nNPY file for layer {}/{} is saved!'.format(h+1,h_points))

#  ==========[ Export dictionary to a CSV file ]==========
    if savecsv_mode:
        csvPath_pc = commonPath + "csv/ConQlt_PC2DR_{}.csv".format(time_stamp)
        csvPath_drone = commonPath + "csv/ConQlt_DR2PC_{}.csv".format(time_stamp)
        csvPath_networks = commonPath + "csv/ConQlt_NET_{}.csv".format(time_stamp)
#        Save 'pc2drone' values to CSV
        ...
        print("(CSV): pc2drone values saved!")
...
#        Save 'drone2pc' values to CSV        
        ...
        with open(csvPath_drone, mode='w') as csv_file:
            ...
            for item in output_dict['drone2pc']:
                writer.writerow(item)
        print("(CSV): drone2pc values saved!")
