...
class Visualiser:
    '''This class is the basis of the following plot classes.'''
# TODO:
...
class heatmapVisualiser(Visualiser):
...
class plotVisualiser(Visualiser):
    '''This class generates scatterplots for every single variable including the polynomial fit line. It is also capable to generate two plots on the same figure of two instances.'''
...
##    Packets
    def plotDist_vs_Tx(obj1,obj2,groupPlot=True):
        sel = 0
        if groupPlot:
            dists = obj1.distances.reshape(1,-1)
            print("dists:\n",dists)
            print("dists.shape",dists.shape)
            arRSSI = obj1.dataList[sel][3].reshape(1,-1)
            anRSSI = obj2.dataList[sel][3].reshape(1,-1)
        else:
