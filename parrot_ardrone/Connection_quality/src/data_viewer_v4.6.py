...
#    anafi_vsArdrone.heatmapAll(groupPlot=True)
#    anafi_vsArdrone.heatmapTx(groupPlot=True)
...
#    GroupPlot
    sel = 11
#    print(anafi.dataList[sel].shape)
#    print(len(anafi.dataList[sel]))
#    print(anafi.dataList[sel])
#    
    fig=plt.figure(figsize=(300,24)) #80*len(anafi.dataList[sel])
    
    for ii,height in enumerate(anafi.layerHeights):
        ax = fig.add_subplot(2,len(anafi.dataList),ii+1)
        
        ax.set_title(anafi.droneName + " ({}m)\n{}".format(height, "var_title"))
        plt.subplots_adjust(left=0.0, right=0.5, bottom=0.5, top=1.0)
        ax.set_xlabel("Distance form PC (meters)")
        ax.set_ylabel("Variable of interest (units)")      
        
        dists = anafi.distances[ii].reshape(1,-1)[0]
        var = anafi.dataList[sel][ii].reshape(1,-1)[0]
        xp = np.linspace(dists.min(),dists.max(),100)
        
        anPoly = np.polyfit(dists,var,2)
        anp = np.poly1d(anPoly)
        plt.plot(dists,var,'.', xp,anp(xp),'-')

##    All graphs in one figure
#    ax = fig.add_subplot(2,len(anafi.dataList),5)
...
#    plt.subplots_adjust(left=0.0, right=0.5, bottom=0.5, top=1.0)
    ...
    dists = anafi.distances.reshape(1,-1)[0]
    var = anafi.dataList[sel].reshape(1,-1)[0]
    xp = np.linspace(dists.min(),dists.max(),100)
