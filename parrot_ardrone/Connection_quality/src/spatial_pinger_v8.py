...
# File paths
csv_file_path = "/home/ros/parrot2_ws/src/parrot_ardrone/Connection_quality/src/csv/ConQlt_{}.csv".format(get_timestamp())
json_file_path = "/home/ros/parrot2_ws/src/parrot_ardrone/Connection_quality/src/JSON/ConQlt_{}.json".format(get_timestamp())
...
for h in range(h_points):
    for w in range(w_points):
        for l in range(l_points):
            ...
            result = ping_parser.parse(transmitter.ping()).as_dict()    # Retreive the ping stats and put them in a dictionary
#            result['point'] = [h, w, l]                                 # Include the cordinates in the dict file
            wifi_stats = wifi_statistics()                              # Retreive wifi statistics

#            Merge wifi_stats and ping dictionaries
            for key, value in wifi_stats.items():
                 result[key] = value
...
#            Save WIFI stats data in the coresponding variables
            ...
            output_list.append({'points':{'z':h,'x':w,'y':l}, 'data': result})
...
#print("=========================================")
#print(output_dict['drone'][0]['data'].keys())
#for point in output_dict['drone']:
#    print(point['data']['rtt_mdev'])

#if savecsv_mode:
#    with open(csv_file_path, mode='w') as csv_file:
#        writer = csv.DictWriter(csv_file,output_dict['drone'][0]['point'] + output_dict['drone'][0]['data'].keys())
#        writer.writeheader()
...
#if savenpy_mode == True:
#    save_npy(fig_data)

#if savejson_mode == True:
#    """Export dictionary to a JSON file"""
##    file_path = "/home/ros/parrot2_ws/src/parrot_ardrone/Connection_quality/src/JSON/ConQlt_{}.json".format(get_timestamp()
#    with open("ConQlt_.json", 'w') as f:
